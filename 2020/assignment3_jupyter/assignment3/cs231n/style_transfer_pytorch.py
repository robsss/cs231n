...
def content_loss(content_weight, content_current, content_original):
    ...
    content_loss = content_weight * torch.sum((content_current
                     - content_original)**2)
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i, style_layer in enumerate(style_layers):
      ...
      style_loss += style_weights[i] * torch.sum((gram_cur
                   - style_targets[i])**2)
...
def tv_loss(img, tv_weight):
    ...
    _, _, H, W = img.shape
    loss = tv_weight * (torch.sum((img[:, :, 1:, :] - img[:, :, :H - 1, :])**2) + 
      torch.sum((img[:, :, :, 1:] - img[:, :, :, :W-1])**2))
